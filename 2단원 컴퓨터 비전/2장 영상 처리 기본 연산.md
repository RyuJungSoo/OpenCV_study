...
즉 한 픽셀이 8비트의 값을 가진다면 **입력 구간[0, 255]를 출력 범위 [0, 255]로 변환** 한 것이다. 이에 반해 구간 변한은 [x<sub>1</sub>, x<sub>2</sub>]를 [y<sub>1</sub>, y<sub>2</sub>]로 변환하며, 이들 사이이ㅢ 관계는 설계자가 임의로 정의할 수 있다.
변환한 것** 이다. 
