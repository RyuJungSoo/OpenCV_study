...
***
**열기**와 **닫기** 연산 - 영상을 전반적으로 어둡거나 밝게 조절하면서 부드러운 느낌을 준다. (열기: 사람의 형상 확대, 닫기: 사람의 형상이 축소)
**그레이디언트 연산** - 팽창을 적용한 영상과 침식을 수행한 영상과의 차이를 통하여 기울기를 구해낸다.             
**탑햇 연산** - 입력 영상과 오픈을 수행한 영상과의 차이를 나타냄.                   
**블랙햇 연산** - 닫기를 수행한 영상과 입력 영상과의 차이를 나타냄.                
**히트미스 연산** - 이미지의 전경이나 배경 픽셀의 특정 패턴을 찾는 데 사용하는 이진 형태학으로서 구조 요소의 형태에 큰 영향을 받는다.
